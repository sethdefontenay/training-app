...
from app.models import DailyWellbeing, SetEntry, User
...
async def test_log_set_tool_creates_session_and_set(session: AsyncSession, user: User) -> None:
    out = await TOOLS_BY_NAME["log_set"].handler(
        session,
        {"date": DAY, "exercise_slug": "leg-press-machine", "reps": "15", "weight": "40"},
        user.id,
    )
...
async def test_set_wellbeing_tool(session: AsyncSession, user: User) -> None:
    await TOOLS_BY_NAME["set_wellbeing"].handler(
        session, {"date": DAY, "energy": 7, "stress": 3}, user.id
    )
...
async def test_get_today_tool_runs_without_plan(session: AsyncSession, user: User) -> None:
    out = await TOOLS_BY_NAME["get_today"].handler(session, {"date": DAY}, user.id)
...
async def test_agent_loop_calls_tool_then_answers(
    ...
    out = await run_chat(
        session, [{"role": "user", "content": "what's on today?"}], user_id=user.id
    )
...
async def test_mcp_call_tool_dispatches(sessionmaker, user: User, monkeypatch) -> None:
    import json
