...
from app.models import Exercise, Measurement, MobilityDone, SetEntry, StepsDay, User
...
async def test_full_import(tmp_path: Path, session: AsyncSession, user: User) -> None:
    vault = _build_vault(tmp_path)
    summary = await import_vault(session, vault, user.id)
...
async def test_bodyweight_set_imports_with_empty_weight(
    tmp_path: Path, session: AsyncSession, user: User
) -> None:
    vault = _build_vault(tmp_path)
